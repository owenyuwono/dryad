...
- **Bark genes** (cosmetic; drive `barkMaterial` uniforms via `setGenome`). The relief is now **furrow-guided FBM** (see "Bark" in the tuning section); the live UI sliders are **`barkHue`** (grey→warm brown/red), **`barkLightness`** (dark→pale/birch), **Furrow density** (the `barkScale` gene → furrow count), and **`barkPlates`** (ridges↔plates morphology — currently a no-op stub after the voronoi removal; rewire it to the plate-break amount if needed). The `barkRelief`/`barkOrient`/`barkLenticels`/`barkShed`/`barkUnderHue` genes still exist in the genome (determinism), but their sliders were **removed** (relief is generated, not slider-driven; lenticels/shed/under-hue are inert for the furrowed-oak look). All bark genes remain in the schema so `randomGenome`/fixtures stay byte-identical.
